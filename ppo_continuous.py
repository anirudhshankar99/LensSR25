...
class Actor(nn.Module):
    ...
    def forward_2(self, X):
        X = X.view(args.num_steps * env.B, env.image_y, env.image_x)
        X = self.model(X)
...
if __name__ == '__main__':
    ...
    selected_galaxies = np.mean(np.load('selected_galaxies.npy'), axis=-1)
    ...
    with tqdm(range(int(args.total_timesteps)), desc=f'episodic_reward: {episodic_reward}') as progress:
        ...
        for i in range(int(args.total_timesteps)):
            ...
            if episodic_reward > best_reward:
                ...
                best_critic_weights = critic.state_dict()
                best_actor_weights = actor.state_dict()
    torch.save(best_critic_weights, 'best_critic.pth')
    torch.save(best_actor_weights, 'best_actor.pth')